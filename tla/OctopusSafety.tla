...
Propose(leader, v) ==
    ...
    /\ \E highQC \in {lockedQC[leader]}:
        ...
        /\ \E parent \in blocks : parent.view = highQC.view  \* G5a: parent must exist
...
UpdateLock(node, qc) ==
    ...
    /\ qc > lockedQC[node].view  \* Monotonicity: only increase
...
FastCommit2Chain(node, v1, v2) ==
    ...
    /\ lockedQC' = [lockedQC EXCEPT ![node] = IF @.view < v2 THEN [view |-> v2, node |-> node] ELSE @]
    /\ UNCHANGED <<view, blocks, votes, timeouts, tcs>>

(* Timeout: a node times out on a view. *)
Timeout(node, v) ==
    /\ v \in 1..VIEWS
...
QCQuorumIntersection == \A v1, v2 \in 1..VIEWS :
    (Cardinality(votes[v1]) >= QuorumSize /\ Cardinality(votes[v2]) >= QuorumSize) =>
