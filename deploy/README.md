# Octopus BFT — Deployment & Evaluation Guide

Complete deployment automation for Octopus BFT: a dynamic, pipelined, multi-leader Byzantine fault-tolerant consensus protocol supporting 1000-node clusters at ≥100k tx/s.
...
The full Octopus pipeline connects two components through a narrow HTTP interface:
- **Go consensus layer** (`src/cmd/octopus`): m parallel pipelined BFT instances with certified chain-internal reconfiguration
...
This will:
1. Build the Go binary (`octopus`, `octopus-genesis`)
...
./build/octopus -id=0 -port=8080 -http=9000 \
...
Each Octopus pod currently requests 500m CPU / 512Mi RAM and is limited to 1000m CPU / 1Gi RAM. For a 1000-node deployment:
...
- **Extra manifest storage**: one read-only PVC named `octopus-manifests` containing `/config/manifests/node-<id>-manifest.json` for every pod

---

## Building the Docker Image

From the **Octopus project root** (parent of `deploy/`):

```bash
# Build the image
docker build -t octopus-bft:latest -f deploy/Dockerfile .

# Verify
docker run --rm octopus-bft:latest --help

# Tag for a registry (optional)
docker tag octopus-bft:latest your-registry.io/octopus-bft:latest
docker push your-registry.io/octopus-bft:latest
```

The Dockerfile uses a **multi-stage build**:
1. **Builder stage** (`golang:1.22-alpine`): compiles `cmd/octopus/main.go` with static linking
...
sed -i 's|octopus-bft:latest|your-registry.io/octopus-bft:latest|' k8s/octopus-statefulset.yaml
```

### Step 2: Prepare runtime manifests

`octopus-statefulset.yaml` now expects a read-only PVC named `octopus-manifests` mounted at `/config/manifests`.
...
3. Create or bind that shared volume to a PVC named `octopus-manifests` in the target namespace.

Without this step, multi-node Octopus startup will fail because the binary rejects ephemeral bootstrap for clusters larger than one node.

### Step 3: Deploy

```bash
# Apply in order: ConfigMap → Service → StatefulSet
kubectl apply -f k8s/octopus-configmap.yaml
kubectl apply -f k8s/octopus-service.yaml
kubectl apply -f k8s/octopus-statefulset.yaml

# Watch rollout (pods start in parallel)
kubectl rollout status statefulset/octopus --timeout=600s

# Check pod status
kubectl get pods -l app=octopus --no-headers | wc -l    # should be 1000
kubectl get pods -l app=octopus | grep -c Running        # healthy count
```
...
kubectl port-forward pod/octopus-0 9000:9000 &
...
kubectl scale statefulset octopus --replicas=100   # small test
kubectl scale statefulset octopus --replicas=500   # medium
kubectl scale statefulset octopus --replicas=1000  # full scale
...
kubectl rollout restart statefulset/octopus
```

### Tear down

```bash
kubectl delete -f k8s/octopus-statefulset.yaml
kubectl delete -f k8s/octopus-service.yaml
kubectl delete -f k8s/octopus-configmap.yaml

# Delete persistent volumes
kubectl delete pvc -l app=octopus
```
...
```bash
cd ../src
go run ./cmd/octopus-genesis -nodes 10 -seed test-cluster -out ../deploy/configs/genesis.json
```
...
Multi-node Octopus now refuses ephemeral bootstrap at runtime. Generate and pass a manifest for every compose, Kubernetes, or bare-metal cluster with more than one node.
...
For Kubernetes, `generate_config.py --k8s` generates the cluster-wide values and the per-node runtime manifests locally, but you must still publish `configs/manifests/` to the `octopus-manifests` PVC consumed by the StatefulSet.
...
Each Octopus node exposes the following HTTP endpoints on its admin port (default 9000):
...
Octopus now includes a separate adaptive control plane that is designed to host future MARL policies such as safety-constrained FACMAC without placing RL logic directly on the consensus hot path.
...
-adaptive-policy-url=http://127.0.0.1:18080/infer
-adaptive-trace-path=/tmp/octopus-trace.jsonl
```
...
1. Octopus exports runtime observations from consensus, orderer, Hydra, and network subsystems.
...
4. Octopus computes a built-in reward signal and optionally writes JSONL trajectories for offline training.
5. Octopus guardrails sanitize the action before it reaches any live engine.
...
Octopus now includes per-instance agent observations in the payload sent to external MARL services.
...
Octopus routes each `agent_action` to the matching consensus instance by `instance_id`, then applies guardrails before tuning that instance.
...
1. Octopus emits traces via `-adaptive-trace-path`
2. The Python service trains from those traces
3. Octopus queries `/infer`
...
- **P2P**: Octopus uses libp2p with GossipSub for message propagation. At 1000 nodes, each node maintains connections to ~20-50 peers (GossipSub mesh), not all 999.
...
Based on the Octopus BFT protocol design (dynamic pipelined multi-leader):
...
```bash
kubectl describe pod octopus-0  # Check events
...
```bash
kubectl exec -it octopus-0 -- curl -s http://localhost:9000/network | jq '.connection'
```

- Check DNS resolution: `kubectl exec octopus-0 -- nslookup octopus-headless`
- Verify headless service: `kubectl get svc octopus-headless`
...
4. Check CPU throttling: `kubectl top pods -l app=octopus --sort-by=cpu`
