...
func main() {
	fmt.Println("═══════════════════════════════════════════════════════════════")
	fmt.Println("  Octopus 1000-Node Scale Benchmark")
	...
	if len(results) > 0 {
		octopus := results[0]
		fmt.Printf("  Octopus@1000: %.0f ktx/s | p50=%.1fms p99=%.1fms | reconfig_overhead=%.1f%%\n",
			octopus.ThroughputKtxs, octopus.LatencyP50Ms, octopus.LatencyP99Ms, octopus.ReconfigOverheadPct)
		fmt.Printf("  Messages:     %s\n", octopus.MessageComplexity)
		fmt.Printf("  BW/node:      %.2f Mbps\n", octopus.BandwidthMbpsPerNode)

		if octopus.ThroughputKtxs >= 100 {
			fmt.Println("  ✓ Throughput target MET (>=100k tx/s)")
		} else {
			fmt.Printf("  ✗ Throughput target MISSED (%.0f < 100k tx/s)\n", octopus.ThroughputKtxs)
		}
		if octopus.LatencyP99Ms <= 100 {
			fmt.Println("  ✓ Latency target MET (p99 <=100ms)")
		} else {
			fmt.Printf("  ✗ Latency target MISSED (p99=%.1fms > 100ms)\n", octopus.LatencyP99Ms)
		}
	}
	fmt.Println()
