...
type octopusAdaptiveRuntime struct {
	nodeID      uint64
...
func (r *octopusAdaptiveRuntime) Observe() adaptive.Observation {
...
func (r *octopusAdaptiveRuntime) Apply(action adaptive.Action) error {
...
func (r *octopusAdaptiveRuntime) applyAgentReconfiguration(action adaptive.AgentAction) error {
...
func (r *octopusAdaptiveRuntime) isCurrentValidator(nodeID uint64) bool {
	if r.memberMgr == nil {
		return false
	}
	cfg := r.memberMgr.GetCurrentConfig()
...
func (r *octopusAdaptiveRuntime) lookupKnownValidator(nodeID uint64) ([]byte, uint64, bool) {
	if r.memberMgr != nil {
...
func (r *octopusAdaptiveRuntime) submitLocalJoinIntent() error {
	return submitLocalJoinIntent(r.nodeID, r.keypair, firstEngine(r.engines), r.memberMgr, r.hydraMgr)
}

func (r *octopusAdaptiveRuntime) submitLocalLeaveIntent() error {
...
func buildAdaptiveController(cfg *bootstrap.EngineConfig, runtime *octopusAdaptiveRuntime) *adaptive.Controller {
...
func (r *octopusAdaptiveRuntime) recordOrderedOutput(out hotstuff.InstanceOutput) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *octopusAdaptiveRuntime) SetScenarioContext(ctx adaptive.ScenarioContext) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.scenario = ctx
}

func (r *octopusAdaptiveRuntime) GetScenarioContext() adaptive.ScenarioContext {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.scenario
}
...
func (r *octopusAdaptiveRuntime) buildTrustSnapshotsWithBayesian(reputation *hotstuff.LeaderReputation) []adaptive.TrustSnapshot {
...
// back to f=1 minimum).
func (r *octopusAdaptiveRuntime) estimateFaults(valSet *types.ValidatorSet) int {
