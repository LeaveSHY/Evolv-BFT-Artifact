...
func main() {
	...
	fmt.Printf("Starting Evolv-BFT Node %d on port %d (HTTP %d), instances=%d, batch=%d, timeout_ms=%d\n", cfg.NodeID, cfg.Port, cfg.HTTPPort, cfg.Instances, cfg.BatchTxs, cfg.TimeoutMs)
	...
	adaptiveRuntime := &evolvbftAdaptiveRuntime{
		nodeID:    cfg.NodeID,
		keypair:   typesKp,
		engines:   engines,
		memberMgr: memberMgr,
