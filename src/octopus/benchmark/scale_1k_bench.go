...
// RunScaleBenchmark simulates the full Octopus system at 1000-node scale.
...
// simulateOptimizedRound models one committed-block interval of Octopus with VRF committee + compression.
...
// CompareAtScale runs Octopus vs baselines at 1000-node scale.
func CompareAtScale(numRounds int) []ScaleResult {
	octopusCfg := Default1000NodeConfig()

	var results []ScaleResult

	// Octopus: 10 instances, VRF committee, compression
	result := RunScaleBenchmark(octopusCfg, numRounds)
	results = append(results, result)
	fmt.Printf("  Octopus (1000 nodes): %.1f ktx/s, p50=%.1f ms, p99=%.1f ms\n",
