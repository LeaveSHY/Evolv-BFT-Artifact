...
func TestConsensusBenchmark_WANScaling(t *testing.T) {
	...
	// Verify Octopus achieves >2x throughput over single-BFT
	for _, n := range replicaCounts {
		var octopusThroughput, singleThroughput float64
		for _, r := range results {
			if r.Config.NumReplicas == n && r.System == "octopus" {
				octopusThroughput = r.ThroughputKtxs
			}
		...
		if singleThroughput > 0 {
			ratio := octopusThroughput / singleThroughput
			t.Logf("n=%d: Octopus/Single-BFT throughput ratio = %.2fx", n, ratio)
			if ratio < 1.5 {
				t.Errorf("n=%d: Octopus should achieve >1.5x over single-BFT, got %.2fx", n, ratio)
			}
		}
	}
}
...
func TestConsensusBenchmark_LANScaling(t *testing.T) {
	...
	// Verify Octopus maintains sub-second latency at 1000 replicas
	for _, r := range results {
		if r.System == "octopus" && r.Config.NumReplicas == 1000 {
			if r.LatencyMs > 1000 {
				t.Errorf("Octopus LAN latency at n=1000 should be sub-second, got %.1f ms", r.LatencyMs)
			}
		}
	}
}
