...
func (n *P2PNetwork) setupDiscovery() error {
	s := mdns.NewMdnsService(n.Host, "evolvbft-discovery", n)
	return s.Start()
}
...
func (n *P2PNetwork) JoinConsensusTopic() error {
	topicName := InstanceConsensusTopic("evolvbft-consensus", 0)
...
func (n *P2PNetwork) JoinMempoolTopic() error {
	ch, err := n.SubscribeTopic("evolvbft-mempool", 8192)
	if err != nil {
		return err
	}
	n.MempoolChan = ch
	n.mu.Lock()
	n.mempoolTopic = n.topics["evolvbft-mempool"]
	n.mempoolSub = n.subs["evolvbft-mempool"]
	n.mu.Unlock()
	return nil
}
