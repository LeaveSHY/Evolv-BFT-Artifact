...
type BenchmarkResult struct {
	...
	System         string          `json:"system"`          // "evolvbft", "single-bft", "bullshark"
...
// simulatePipelinedRound models one round of Evolv-BFT pipelined BFT.
...
func RunScalingSuite(network NetworkProfile, replicaCounts []int, numInstances int, numRounds int) []BenchmarkResult {
	...
	systems := []string{"evolvbft", "single-bft", "bullshark", "ladon"}
...
func RunReconfigBenchmark(network NetworkProfile, batchSizes []int, numRounds int) []ReconfigResult {
	...
	for _, batchSize := range batchSizes {
		for _, sys := range []string{"evolvbft", "single-bft"} {
			cfg := ConsensusConfig{
				NumReplicas:   100,
				NumInstances:  4,
				FaultFraction: 0.33,
