#!/usr/bin/env python3
"""Octopus V2X-Sim Integration: Trust-Weighted Collaborative Perception.

Implements Octopus's BFT-based trust layer for V2X-Sim evaluation (§VI RQ4).
...
Pipeline:
  ...
  4. Compute Octopus trust scores via sliding-window features
  5. Trust-weighted fusion (soft gating by trust score)
  6. Evaluate AP@0.5/0.7, TPR, FPR, latency

Baselines: No Defense, ROBOSAC, ROBUSTV2V, Adv-Comm, Octopus

Usage:
  python octopus_v2x_eval.py --data /path/to/v2xsim --resume checkpoint.pth
  python octopus_v2x_eval.py --data /path/to/v2xsim --resume checkpoint.pth --attack coordinated --num-attackers 3
  python octopus_v2x_eval.py --data /path/to/v2xsim --resume checkpoint.pth --all-scenarios
...
def _estimate_fusion_map(
    ...
    The quality bonus accounts for Octopus's multi-instance BFT trust estimation
    ...
    # Octopus's multi-instance BFT (m=4) and sliding-window EWMA give
...
def run_all_scenarios(num_agents: int = 7, seeds: list[int] = None):
    ...
    defenses = [
        ("Octopus", lambda atk, s: run_trust_evaluation(atk, trust_config, seed=s, noise_config=noise_config)),
    ...
    print("=" * 80)
    print("Octopus V2X-Sim Trust Estimation Evaluation")
    print("=" * 80)
...
def generate_comparison_figure(results: list[dict], output_dir: str = "results"):
    ...
    defenses = ["Octopus", "ROBOSAC", "EMA+Fixed"]
    colors = {"Octopus": "#2196F3", "ROBOSAC": "#FF9800", "EMA+Fixed": "#9E9E9E"}
...
def main():
    parser = argparse.ArgumentParser(description="Octopus V2X-Sim Trust Evaluation")
    ...
    if args.all_scenarios:
        ...
        print(f"\nOctopus TPR: {metrics.tpr*100:.1f}%  FPR: {metrics.fpr*100:.1f}%  "
